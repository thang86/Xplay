...
public class ResponseCache {
    ...
    public void insert(String key, List<MoviePopular> list) {
        Parcel parcel = Parcel.obtain();
        parcel.writeList(list);
        saveValue(key, parcel);
    }

    public List<MoviePopular> get(String key) {
        List<MoviePopular> list = new ArrayList<>();
        Parcel parcel = getParcel(key);
        if (parcel != null) {
            try {
                parcel.readList(list, MoviePopular.class.getClassLoader());
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                parcel.recycle();
